...
if (MODULES.filter((module) => module.status === 'live').map((module) => module.id).join(',') !== 'escape,wear,watch,move,eat') {
  throw new Error('Escape, Wear, Watch, Move and Eat should be the live consumer modules in this platform batch.');
}
...
if (!master.moduleIds.includes('eat')) throw new Error('Eat is missing from the master module ID set.');
...
for (const id of ['escape', 'wear', 'watch', 'move', 'eat']) {
  if (!progress.find((module) => module.id === id)?.completed) throw new Error(`${id} completion is missing from module progress.`);
}
if (progress.find((module) => module.id === 'live')?.completed) throw new Error('Planned modules must not appear completed.');

const html = fs.readFileSync(new URL('../index.html', import.meta.url), 'utf8');
for (const asset of ['platform.css', 'platform.js', 'wear.css', 'wear.js', 'watch.css', 'watch.js', 'move.css', 'move.js', 'eat.css', 'eat.js']) {
...
console.log(`Platform OK — ${MODULES.length} modules registered, Escape + Wear + Watch + Move + Eat live, ${MASTER_DIMENSIONS.length}D master profile, five-domain Passport aggregation wired.`);
