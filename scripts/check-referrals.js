...
if (!report.sample_ready) throw new Error('Referral reporting should unlock rate review at the configured sample minimum.');
...
const below = referralAggregate(events.filter((event) => !event.properties?.referral_token || Number(event.properties.referral_token.match(/\d+/)?.[0] || 0) < MIN_REFERRAL_SAMPLE - 1));
if (below.creator_tokens >= MIN_REFERRAL_SAMPLE && below.sample_ready) throw new Error('Referral rate gate must not unlock below the minimum creator-token sample.');
...
for (const marker of ['tasteprint_referral_stats', 'creator_tokens', 'tokens_opened', 'secondary_share_sessions', 'grant execute', 'to anon']) {
...
for (const marker of ['tasteprint_referral_stats', 'referralAggregate', 'token_activation_pct', 'same_session_reshare_pct']) {
...
console.log(`Referral attribution OK — privacy-safe cross-device aggregate scaffold, ${MIN_REFERRAL_SAMPLE}-token rate gate, share outcomes and downstream resharing metrics wired.`);
